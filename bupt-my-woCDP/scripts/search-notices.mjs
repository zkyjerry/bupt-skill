 ...
 *
 * 用法：
 *   node search-notices.mjs --keyword <关键词> [--page N] [--list-only] [--json] [--download-dir DIR]
 *
 * 选项：
...
const LIST_URL = "http://my.bupt.edu.cn/list.jsp?urltype=tree.TreeTempUrl&wbtreeid=1154";
  ...
  try {
    ...
    if (isListOnly) {
      if (isJson) {
        ...
        console.log(`校内通知${keyword ? ` - 搜索"${keyword}"` : ""}（第 ${page} 页，共 ${matches.length} 条）\n`);
    ...
    if (matches.length === 0) {
      if (isJson) {
        ...
        console.log(`未找到包含"${keyword}"的校内通知`);
        console.log(`\n当前页面的列表（可能相关）：`);
